...
{					     
	...
	GPIO_SetBits(GPIOB,GPIO_Pin_1|GPIO_Pin_2); 	//PB1,PB2 输出高
}
//产生IIC起始信号
void APDS9960_IIC_Start(void)
{
	...
	delay_us(4);
	APDS9960_IIC_SCL=0;//钳住I2C总线，准备发送或接收数据 
}	  
//产生IIC停止信号
void APDS9960_IIC_Stop(void)
{
	...
	APDS9960_IIC_SDA=0;//STOP:when CLK is high DATA change form low to high
 	delay_us(4);
	APDS9960_IIC_SCL=1; 
	APDS9960_IIC_SDA=1;//发送I2C总线结束信号
	delay_us(4);							   	
}
//等待应答信号到来
//返回值：1，接收应答失败
...
u8 APDS9960_IIC_Read_Byte(unsigned char ack)
{
    ...
    if (!ack)
        APDS9960_IIC_NAck();//发送nACK
    else
        APDS9960_IIC_Ack(); //发送ACK   
    return receive;
}
