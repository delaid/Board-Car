...
{ 	
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD, ENABLE);
	...
	GPIO_InitStructure.GPIO_Pin = S24R1_CS_PIN|S24R1_CE_PIN;	//Pd8 9 推挽 
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(S24R1_CE_PORT, &GPIO_InitStructure);//初始化指定IO
  
	GPIO_InitStructure.GPIO_Pin  = S24R1_IRQ_PIN;   
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //Pd10 输入  
	GPIO_Init(S24R1_IRQ_PORT, &GPIO_InitStructure);

	GPIO_ResetBits(S24R1_IRQ_PORT,S24R1_CS_PIN|S24R1_CE_PIN|S24R1_IRQ_PIN);//Pd8,9,10上拉					 ``
